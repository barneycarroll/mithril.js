...
If it's undesirable for a given part of the application to participate in redraw, this can be achieved by returning `false` from the [`onbeforeupdate`](lifecycle-methods.md#onbeforeupdate) hook. For example, the component below will only draw once, and then never redraw. More complex logic can be inserted
